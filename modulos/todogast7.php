...
</body>   <script src="jquery/jquery-3.3.1.min.js"></script>
...
if(isset($eliminar)){
  ...
  $mysqli->query("DELETE FROM pedido7 WHERE id = '$eliminar'");
      alert("Pedido eliminado",1,'todogast7');
}
    $so1 = $mysqli->query("SELECT * FROM pedido7 WHERE id = '$ide' ");
    $ro1 = mysqli_fetch_array($so1);
              if(!isset($dia) && !isset($semana) && !isset($mes)){
     $q24 = $mysqli->query("SELECT * FROM pedido7 WHERE id_cliente= '$id_cliente' AND tipoid=7   ORDER BY fecha ASC"); 
...
?>
                        ...
                        <div class="col-12 col-lg-9 mt-4 mt-lg-0">
                            <div class="card">
                                <div class="card-header" style="    display: flex;">
                                    <h5>Pedidos</h5><a style="color:white">aaaa</a>
                                      <button  style="padding: -1px;
    width: 45%!important;" name="enviar1" type="submit" class="btn btn-primary">
                                                    <a style="    font-size: 13px;color:white" href="?p=verprove">Ver Proveedores</a>
                                                </button>
                                </div>
    ...
    display: contents;
">                                    
         <a style="padding:5px">Organiza y administra tus pedidos</a> 
         <br><br>
...
?>
            ...
            <td style="text-align: center;"><?=$r24['fecha2']?></td>
            <td style="text-align: center;"><?=$r24['distribuidor']?></td>
            <td style="text-align: center;">$ <?=number_format($r24['valor'])?></td>
             <td style="text-align: center;"><?=$r24['fecha']?></td>
            <td style="text-align: center;"><a href="?p=gasto17&ide=<?=$r24['id']?>"><i class="fas fa-question-circle"></i></a></td>
            <td style="text-align: center;"><a href="?p=todogast7&eliminar=<?=$r24['id']?>"><i class="fas fa-times-circle"></i></a></td>
