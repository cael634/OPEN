...
<ul style="border:1px solid">
  <li ><a href="?p=estadistica" style="background:white">Estadistica de ventas</a></li>
  <li>
  
  <li><a href="?p=productosestadisticasuni" style="background:white">Estadisticas por productos</a></li>
  <li class="current-menu-item"><a href="?p=comparproductos">Comparacion de ventas por producto</a></li>
  <li>
    ...
    max-width: 100%;">
              ...
              <div class="card shadow mb-4">
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary"style="color: #000619!important;">Ventas <?=$diar?></h6>
                </div>
                <div class="card-body">
                  <?php
                  $mes = date('m');
...
$contador11= 0;
$contadorto= 0;
$sx = $mysqli->query("SELECT * FROM compven WHERE DAY(fecha) = '$diax' AND MONTH(fecha) = '$mesx' AND YEAR(fecha) = '$aniox' ");
...
$contadoruni=(100/$contadorto)*$contador11;
  ...
  ?>
                  ...
                  <h4 class="small font-weight-bold"><?=$rl['name']?> (Total de unidades vendidas: <?=$contador11?>)<span class="float-right"> <?php
...
?>
    ...
    max-width: 100%;">
              ...
              <div class="card shadow mb-4">
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary" style="color: #000619!important;">Ventas <?=$mesro?> del <?=$anior?></h6>
                </div>
                <div class="card-body">
<?php
...
$contador11= 0;
$contadorto= 0;
$sx = $mysqli->query("SELECT * FROM compra WHERE MONTH(fecha) = '$mesr' AND YEAR(fecha) = $anior");
while($rx =mysqli_fetch_array($sx)){
$compra = $rx['id'];
$sd = $mysqli->query("SELECT * FROM productos_compra WHERE id_compra = '$compra' AND id_vendedor = '$id_vendedor' ");
while($rd =mysqli_fetch_array($sd)){
...
?>
 ...
 <div class="row">
    ...
    max-width: 100%;">
              ...
              <div class="card shadow mb-4">
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary" style="color: #000619!important;">Ventas de <?=$primerDia?> hasta <?=$ultimoDia?>.
                   Semana <?=$semanau?> </h6>
                </div>
                <div class="card-body">
<?php
...
$contador11= 0;
$contadorto= 0;
$sx = $mysqli->query("SELECT * FROM compven WHERE semana = '$semanau' AND YEAR(fecha) = $aniox");
while($rx =mysqli_fetch_array($sx)){
$compra = $rx['compra'];
$sd = $mysqli->query("SELECT * FROM productos_compra WHERE id_compra = '$compra' AND id_vendedor = '$id_vendedor' ");
while($rd =mysqli_fetch_array($sd)){
...
<?php
  ...
  ?>
          <div class="row">
            ...
            <!-- Content Column -->
            <div class="col-lg-6 mb-4" style="    flex: 0 0 100%;
    max-width: 100%;">

              <!-- Project Card Example -->
              <div class="card shadow mb-4">
                <div class="card-header py-3">
                  <h6 class="m-0 font-weight-bold text-primary" style="color: #000619!important;">Porcentaje de ventas este mes</h6>
                </div>
                <div class="card-body">
                  <?php
...
if($contador32>0){
                  $mes = date('m');
                  $id_vendedor = clear($_SESSION['id_vendedor']);
$sll = $mysqli->query("SELECT * FROM productosemp WHERE id_vendedor= '".$_SESSION['id_vendedor']. "' ");
while($rll =mysqli_fetch_array($sll)){
$productos = $rll['id'];
$contador11= 0;
$contadorto= 0;
$sxx = $mysqli->query("SELECT * FROM compra WHERE MONTH(fecha) = '$mes' ");
while($rxx =mysqli_fetch_array($sxx)){
$comprax = $rxx['id'];
$sdx = $mysqli->query("SELECT * FROM productos_compra WHERE id_compra = '$comprax' AND id_vendedor = '$id_vendedor' ");
while($rdx =mysqli_fetch_array($sdx)){
  $contadorto++;
}
$syx = $mysqli->query("SELECT * FROM productos_compra WHERE id_compra = '$comprax' AND id_producto = '$productos' ");
while($ryx =mysqli_fetch_array($syx)){
  $contador11++;
}
}
