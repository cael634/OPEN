...
if(isset($eliminar)){
  ...
  $mysqli->query("DELETE FROM pedido7 WHERE id = '$eliminar'");
      alert("Factura eliminada",1,'todogast');
}
    $so1 = $mysqli->query("SELECT * FROM pedido7 WHERE id = '$ide' ");
    $ro1 = mysqli_fetch_array($so1);
        if(!isset($dia) && !isset($semana) && !isset($mes)){
     $q24 = $mysqli->query("SELECT * FROM pedido7 WHERE id_cliente= '$id_cliente' AND tipoid=6   ORDER BY fecha ASC"); 
    
}
if(isset($mes)){
 $q24 = $mysqli->query("SELECT * FROM pedido7 WHERE id_cliente= '$id_cliente'AND MONTH(fecha) = '$mes' AND tipoid=6 ORDER BY fecha ASC");   
}
if(isset($dia)){
    $diax   = substr($dia,8,2);
$mesx = substr($dia,5,2);
...
$semanax = date('W',  mktime(0,0,0,$mesx,$diax,$aniox));  
 $q24 = $mysqli->query("SELECT * FROM pedido7 WHERE id_cliente= '$id_cliente' AND MONTH(fecha) = '$mesx'AND YEAR(fecha) ='$aniox' AND DAY(fecha)='$diax' AND tipoid=6 ORDER BY fecha ASC");   
}
...
$semanax = date('W',  mktime(0,0,0,$mesx,$diax,$aniox));  
 $q24 = $mysqli->query("SELECT * FROM pedido7 WHERE id_cliente= '$id_cliente' AND semana= '$semanax' AND tipoid=6  ORDER BY fecha ASC");  
...
?>
    ...
    display: contents;
">                                    
         <a style="padding:5px">Organiza y recuerda tus facturas</a> 
         <br><br>
...
?>
            ...
            <td style="text-align: center;"><?=$r24['descripcion']?></td>
             ...
             <td style="text-align: center;"><a href="?p=gasto16&ide=<?=$r24['id']?>"><i class="fas fa-question-circle"></i></a></td>
             <td style="text-align: center;"><a href="?p=todogast&eliminar=<?=$r24['id']?>"><i class="fas fa-times-circle"></i></a></td>
