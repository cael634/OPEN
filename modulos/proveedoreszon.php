...
if(isset($enviar1)){
    ...
    $so112 = $mysqli->query("SELECT * FROM admin WHERE select7 = '$enviar1' ");
    $ro112 = mysqli_fetch_array($so112);
    if(mysqli_num_rows($so112)>0){
    $mysqli->query("INSERT INTO vendedoraña (id_vendedor, id_cliente) VALUES ('".$ro11['id']."','$id_cliente')");
    alert("Se añadio correctamente",1,'proveedoreszon');
}else{
    alert("No existe el codigo",2,'proveedoreszon');
}
}
?>
                            ...
                            <div class="account-nav flex-grow-1" style="    max-height: 350px;">
                                ...
                                <ul>
                                     ...
                                     <li class="account-nav__item  account-nav__item--active ">
                                        <a href="">Proveedores de tu zona</a>
                                    </li>
                                     <li class="account-nav__item  account-nav__item ">
                                        <a href="?p=proveedoreszon2">Proveedores añadidos</a>
                                    </li>
    ...
    display: contents;
">                                    
         <a style="padding:5px" style="    font-size: 13px;">Codigo del proveedor (este codigo es entregado por el proveedor para añadirlo automaticamente a tu lista)</a> 
         <br><br>
     
                                                    
            <input style="width: 45%;" type="text" class="form-control" name="descripcion" placeholder=" Ingresalo">
                <a style="color:white">a</a>
                    <button  style="padding: -1px; width: 43%!important;" name="enviar1" type="submit" class="btn btn-primary">                                                  Añadir
                    </button>
              </form>
                                 
              Recuerda <strong style="color:#e2013b;"> &nbsp;ahórra tiempo y trabajo</strong> Digitaliza gratis tus cuentas : <a href="https://open.com.se/?p=gasto" style="color:#007bff;    display: contents;">Ir</a>                       
                                                 <form method="post" action="" style="
    display: contents;
">  
    ...
    width: 47%!important;" name="enviar1" type="submit" class="btn btn-primary">
           ...
           <a style="padding:5px" style="    font-size: 13px;">Si no tines un codigo, no te procupes añade los proveedores de tu zona a tu lista</a> 
...
<?php
 ...
 $q24 = $mysqli->query("SELECT * FROM admins WHERE departamento = '$hola'"); 
while($r24=mysqli_fetch_array($q24)){
   $z12 = $mysqli->query("SELECT * FROM vendedoraña WHERE id_cliente =  '$id_cliente' AND id_vendedor ='".$r24['id']."'");
    if(mysqli_num_rows($z12)==0){  
...
while($r24=mysqli_fetch_array($q24)){
    echo 1;
   $z12 = $mysqli->query("SELECT * FROM vendedoraña WHERE id_cliente = 32");
   while($x12=mysqli_fetch_array($z12)){
       echo 1;
       if($r24['id']==$x12['id_vendedor']){
           echo 1;
  }else{
      echo 2;-->                    <tr>
            ...
            <td style="text-align: center;"><?=$r24['name']?></td>       
            <td style="text-align: center;"><?=$r24['municipio']?></td>
     
             <td style="text-align: center;"><a href="?p=proveedoreszon&ide=<?=$r24['id']?>&com=1"><i class="fas fa-plus-circle"></i></a></td>
