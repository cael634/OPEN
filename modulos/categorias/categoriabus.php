...
if(isset($agregar) && isset($cant)){
   ...
   $sc = $mysqli->query("SELECT * FROM categorias WHERE id = '$id'");
    $rc = mysqli_fetch_array($sc);
    ?>
...
$ro = mysqli_fetch_array($so);
      ...
      <a class="product--price" >
        ...
        if($r['oferta']>0){
            ?>
            <div class="price--rrp"><?=$divisa?><?=number_format($preciototal)?> </div>
          <div class="price--sell-price"><?=$divisa?><?=number_format($r['price'])?></div>
